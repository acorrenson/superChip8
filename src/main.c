...
int main(int argc, char const *argv[]) {
  ...
  if ( pWindow ) {
    p("Successfully created superChip8 window\n");
    ...
    while ( !quit ) {
      ...
      if (opCode == 0x00E0){
        ...
        p("%04X - SNE V%d (%d) V%d (%d)\n", opCode, x, V[x], y, V[y]);
        PC += V[x] != V[y] ? 4 : 2;
      }
      
      else if ((opCode & 0xF000) == 0xA000) {
        p("%04X - LD I %d [set I = %d]\n", opCode, nnn, nnn);
        I = nnn;
        PC += 2;
      }
